...
class LogInVars:
    ...
    def user_name_val_send(self, username):
        self.driver.find_element(by=By.ID, value=LogInVars.user_name_loc).send_keys(username)
        time.sleep(3)

    def password_val_send(self, password):
        self.driver.find_element(by=By.ID, value=LogInVars.password_loc).send_keys(password)
        time.sleep(3)
    ...
    def assert_control_user_name(self):
        return self.driver.find_element(by=By.XPATH, value=LogInVars.control_user_name).text
